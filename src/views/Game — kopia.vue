...
<div>
  ...
  <section id="game__book" v-bind:style="Zoomstyle">
    <book @getpassword="sendpassword" ref="book" :onLoad="isloaded()" @openpopup="openpopup" v-bind:style="Zoomfirefox"></book>
  </section>
  <transition name="fade">
    <Transitionn v-if="ShowModal"></Transitionn>
  </transition>                               <!-- // dodałem tutaj zoom funkcje v-bind -->
  <section id="game__keyboard" v-bind:class="{ active: ShowPage }" v-bind:style="Zoomstyle">
    <keyboard :password="Password" :passwordlen="Passwordlen" @passwordcorect="passwordcorect" @openpopup="openpopup" v-bind:style="Zoomfirefox"></keyboard>
  </section>
</div>
</template>
  ...
  methods: {
    ...
    changezoom: function () {
      if (window.innerWidth < 1600 && window.innerWidth > 1250) {
        ...
        this.Zoomfirefox = { '-moz-transform': 'scale(75%)' }
      } else if (window.innerWidth < 1250 && window.innerWidth > 800) {
        this.Zoomstyle.zoom = '60%'
        this.Zoomfirefox = { '-moz-transform': 'scale(60%)' }
      } else {
